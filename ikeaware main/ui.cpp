...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	const ImGuiID id = window->GetID(label);
	ImVec2 size = ImGui::CalcItemSize(size_arg, ImGui::CalcItemWidth(), style.FramePadding.y * 2.0f + GetFont()->FontSize);
	...
	std::string buf_display = label;
	buf_display = buf_display.substr(0, buf_display.find("##"));
	buf_display += ": ";
	...
	ImGui::RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, buf_display.c_str(), NULL, NULL, style.ButtonTextAlign, &clip_rect);
