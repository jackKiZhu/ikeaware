...
void InputSys::Initialize()
{
	...
	{
		MessageBoxA(nullptr, "GetCreationParameters failed.", "ERROR", MB_OK);
		exit(EXIT_FAILURE);
	}
	...
	if (!m_ulOldWndProc)
	{
		MessageBoxA(nullptr, "SetWindowLongPtr failed.", "ERROR", MB_OK);
		exit(EXIT_FAILURE);
	}
}
...
LRESULT __stdcall InputSys::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (g_Menu->IsVisible() && ImGui_ImplWin32_WndProcHandler(hWnd, msg, wParam, lParam))
		return true;

	return CallWindowProc((WNDPROC)g_InputSys->m_ulOldWndProc, hWnd, msg, wParam, lParam);
}
